...
export const Particles: React.FC<ParticlesProps> = ({
  ...
  const animate = () => {
    ...
    const activeCircles = circles.current.slice();

    for (let i = 0; i < activeCircles.length; i++) {
      for (let j = i + 1; j < activeCircles.length; j++) {
        const p1 = activeCircles[i];
        const p2 = activeCircles[j];
        ...
        if (distance < connectionRadius) {
          if (context.current) {
            ...
            // Also factor in particle opacities
            const mergedAlpha = (p1.alpha + p2.alpha) / 2;

            context.current.strokeStyle = `rgba(${rgb.join(', ')}, ${(opacity * 0.15 * mergedAlpha).toFixed(3)})`;
