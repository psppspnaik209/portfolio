  ...
  experiences: [
    {
      ...
      to: 'Present',
      highlights: [
        'Developed user interfaces using React and TypeScript to render real-time security alert dashboards (UI/UX) and threat metrics fetched from asynchronous backend Django REST endpoints.',
        'Integrated OpenAI APIs into a Node.js backend to generate automated system log summaries, implementing retry logic, exponential backoff, and fallback handling for rate limits.',
        'Refactored database access patterns in PostgreSQL by replacingnested ORM queries with explicit SQL relational joins and compound indexes across high-traffic table columns.',
        'Utilized AI assisted tools to generate unit and integration test suites in PyTest and Jest, expanding code coverage across user authentication, token refresh, and account management API routes.',
      ],
      stack: [
        'React',
        'TypeScript',
    ...
    },
    {
      ...
      to: 'May 2025',
      highlights: [
        'Engineered Java and Spring Boot microservices for user authentication and authorization workflows, caching active session tokens and user permissions in Redis to reduce primary database query load.',
        'Containerized Spring Boot microservices using Docker and automated application deployments to Azure Kubernetes Service (AKS) using ArgoCD and GitOps release workflows.',
        'Configured distributed tracing, APM monitors, and custom log alerts in Datadog across backend microservices to isolate request latency bottlenecks and diagnose root causes during production incidents.',
        'Secured external REST APIs by implementing OAuth2 authentication and Role-Based Access Control (RBAC) middleware to enforce granular permission checks on incoming HTTP requests.',
      ],
      stack: [
        'Java',
        'Spring Boot',
    ...
    },
    {
      ...
      to: 'July 2024',
      highlights: [
        'Automated CI/CD deployment workflows using Python scripts and GitHub Actions to run PyTest suites, linting checks, and static security scans on every pull request prior to merging code.',
        'Built a command-line diagnostic tool in C++ and Python for Linux environments to parse raw TCP/IP packet logs and convert network telemetry into structured JSON for local debugging.',
        'Authored technical design documents (RFCs) detailing system architecture, database schema migrations, REST API data contracts, and error-handling strategies prior to writing production code.',
        'Conducted peer code reviews on GitHub repositories, evaluating incoming pull requests for edge-case handling, input validation, test coverage, and adherence to team coding standards.',
      ],
      stack: [
        'Python',
        'GitHub Actions',
